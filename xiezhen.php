...
<body>
    <div class="container">
        <header>
            ...
            <nav>
                <ul>
                    ...
                    <li><a href="xiezhen.php" class="active">写真</a></li>
                </ul>
            </nav>
        </header>
        
        <h2 class="section-title">精选写真集</h2>
        
        <section class="photo-grid" id="photoGrid">
            <!-- 照片卡片将通过JavaScript动态加载 -->
 </section>
        ...
        <div class="modal-overlay" id="modalOverlay">
            <div class="purchase-modal">
                <h2 id="modalTitle">获取《抖音 火龙果羊 微密圈 NO.038期 【5V】》写真集</h2>
                <div class="subtitle">登录白嫖动漫/漫画/游戏/真人</div>
    ...
    <script>
            ...
            fetch('./txt/123.txt')
                ...
                .then(data => {
                    ...
                    lines.forEach((line, index) => {
                        if (line.trim()) {
                            const [imgUrl, title, count, rating] = line.split('|');
                            const photoId = index + 1;
                            
                            const photoCard = document.createElement('div');
                            photoCard.className = 'photo-card';
                            photoCard.setAttribute('data-photo', photoId);
                            
                            photoCard.innerHTML = `
                                <div class="premium-badge">VIP</div>
                                <img src="${imgUrl}" class="photo-thumbnail" alt="写真${photoId}">
                            ...
                            photoCard.addEventListener('click', function() {
                                modalTitle.textContent = `获取《${title}》写真集`;
        ...
        // 页面加载时初始化
        window.addEventListener('DOMContentLoaded', loadPhotoData);
        
        // 加载更多按钮点击
        loadMoreBtn.addEventListener('click', function() {
            // 显示购买弹窗
            modalTitle.textContent = "主打一个白嫖";
            modalOverlay.classList.add('active');
            
            // 按钮动画效果
            this.innerHTML = '加载中...';
            setTimeout(() => {
                this.innerHTML = '加载更多写真';
            }, 1500);
        });
